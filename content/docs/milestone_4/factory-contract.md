...
然而，tick 间隔越大，精度越低。价格波动性低的交易对（例如两种稳定币的池子）需要更高的精度，因为在这样的池子中价格移动很小；价格波动性中等和较高的交易对可以有更低的精度，因为在这样的交易对中价格移动会很大。为了处理这样的对阳性，Uniswap 允许在交易对创建时设定一个 tick 间隔。Uniswap 允许部署者在下列选项中选择：10，60，200，而简单起见我们的实现中只考虑10和60。
